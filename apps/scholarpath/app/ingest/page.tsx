  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10">
      <header className="space-y-2">
        <p className="text-sm uppercase tracking-[0.2em] text-amber-300">Admin Workflow</p>
        <h1 className="text-3xl font-semibold">AI Ingest</h1>
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 rounded-3xl border border-slate-800 bg-slate-900 p-6">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-sm text-slate-300">
            Title
            <input
              className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-950 px-4 py-2 text-white"
              value={form.title}
        ...
        <label className="text-sm text-slate-300">
          Raw source text
          <textarea
            className="mt-2 min-h-64 w-full rounded-2xl border border-slate-800 bg-slate-950 px-4 py-3 text-white"
            value={form.raw_text}
        ...
        <div className="flex items-center gap-3">
          <button
            type="submit"
            disabled={loading}
            className="rounded-2xl bg-amber-400 px-4 py-2 font-semibold text-slate-950 disabled:opacity-60"
          >
          ...
          <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
            ...
            <div className="mt-4 space-y-3">
              {result.extraction.metrics.map((metric) => (
                <div key={`${metric.stat_name}-${metric.id}`} className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
