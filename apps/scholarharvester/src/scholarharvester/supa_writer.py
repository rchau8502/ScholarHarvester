...
def supabase_conn():
    try:
        ...
        raise RuntimeError(
            "Postgres client is not installed. Install project dependencies with Poetry "
            "or add `psycopg[binary]` to the environment."
        ) from exc
...
def upsert_dataset(connection: Any, payload: DatasetPayload, source_name: str) -> int:
...
def upsert_metric(connection: Any, dataset_id: int, payload: MetricPayload) -> int:
    cols = [
        "dataset_id",
        "campus",
        "major",
...
def upsert_citation(connection: Any, metric_id: int, citation: CitationPayload) -> None:
    with connection.cursor() as cur:
        cur.execute(
            """
